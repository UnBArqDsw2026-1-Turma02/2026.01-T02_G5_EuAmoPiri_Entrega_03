...
O padrão de projeto Proxy é um padrão estrutural que fornece um substituto ou marcador de posição para outro objeto, controlando o acesso a ele (REFACTORING GURU, 2026). A intenção principal do Proxy é criar um intermediário que gerencia o acesso ao objeto real, permitindo adicionar funcionalidades extras, como controle de permissões, validação de requisições, *lazy loading* ou *cache*, antes de encaminhar a solicitação ao objeto verdadeiro (SOURCEMAKING, 2026). Conforme apresentado pela Profa. Milene Serrano, este padrão é fundamental quando se deseja adicionar uma camada de segurança e controle sem modificar o código do objeto original (SERRANO, 2026).

No contexto de plataformas interativas, a utilização do padrão Proxy atua como um "guardião": intercepta as requisições antes de repassá-las aos serviços de banco de dados, aplicando regras de negócio e restrições (REFACTORING GURU, 2026). Essa abordagem está em estrita conformidade com o Princípio da Responsabilidade Única (SRP) do SOLID, uma vez que separa a lógica de validação de dados da lógica de persistência, aumentando a manutenibilidade e a segurança do software (SERRANO, 2026).
...
A metodologia para a aplicação do padrão de projeto Proxy neste trabalho seguiu etapas de análise estrutural, comunicação assíncrona entre executoras, modelagem, implementação e revisão de outros participantes do grupo.
...
As executoras **Amanda de Moura** e **Gabriela Dourado** se comunicaram a construção do Proxy pelo canal `#proxy_gof_estruturais`, na plataforma Discord, de forma assíncrona, registrando decisões de arquitetura, modelagem UML e refinamento de código. Abaixo, o resumo cronológico de cada troca relevante.
...
*Figura 2 — Estrutura Proxy refinada para o cadastro de relatos (versão 2).*

![estrutura do Proxy (v2)](assets/proxy_estrutura_v2.png)

Fonte: Discord

> Arquivo na íntegra (`.drawio`) da Figura 2 disponível no Google Drive: [UML_Estrutura_Proxy_v2.drawio](https://drive.google.com/file/d/1N_sLrZpYUUEe0TXP70SXuvdBIRJIgK5n/view?usp=sharing).
...
A revisão do artefato foi realizada de forma assíncrona no canal `#proxy_gof_estruturais` do Discord, em continuidade às discussões registradas na metodologia.

#### Revisão — Mariana

*Figura 3 — Print da revisão da Mariana, com aprovação do artefato e validação da saída do `proxy.py`.*

![Revisão da Mariana no canal `#proxy_gof_estruturais`](imagens/proxy-revisao-mariana.png)

Fonte: Discord

**Resumo:** Mariana revisou o artefato consolidado da v2.0, executou mentalmente o código `proxy.py` e compartilhou o print da saída obtida, confirmando o comportamento esperado do padrão. Em sua mensagem, destacou que "a saída está certinha" e que, mesmo não sendo fã de Python, conseguiu acompanhar a implementação, observando claramente a **intermediação realizada pelo Proxy** — tanto no fluxo bloqueado (`[Proxy] BLOQUEADO: Usuário não autenticado`) quanto no fluxo aprovado (`[Proxy] ✓ Validações aprovadas. Repassando para o Serviço Real...` seguido de `[Serviço Real] Salvando o relato do turista 101 no banco de dados do EuAmoPiri...`). A revisão validou o papel do Proxy como interceptador das requisições antes da persistência, sem solicitar ajustes adicionais ao código ou à modelagem.


#### Revisão — Milena

*Figura 4 — Print da revisão da Milena, destacando a validação da modelagem, o refinamento do código e a solicitação de ajuste na padronização das imagens.*

![Revisão da Milena no canal `#proxy_gof_estruturais`](imagens/proxy-revisao-milena.png)

Fonte: Discord

**Resumo:** Milena revisou o artefato do padrão Proxy e validou que a implementação foi realizada de forma correta e bem contextualizada no sistema do EuAmoPiri. Em sua análise, ela destacou o alinhamento consistente entre a modelagem UML, a documentação e o código, evidenciando claramente o papel do Proxy como intermediário responsável por realizar validações (autenticação por token, controle anti-spam, limite de caracteres e filtragem de conteúdo) antes de acionar o serviço real de persistência. Ela também elogiou a evolução técnica entre as versões v1 e v2, apontando que a separação dessas responsabilidades reduziu o acoplamento e melhorou a segurança da aplicação, aproximando o projeto de cenários reais de software. O único ajuste solicitado foi em relação à padronização das imagens no documento, orientando que o título e a descrição fiquem acima da figura, e a fonte de autoria abaixo, utilizando a documentação do padrão Builder como base.

---

## Vídeo de demonstração do Proxy

<iframe width="1280 " height="633" src="https://www.youtube.com/embed/g-twS5qU_KU?si=rNDa5mzHy4waOmIe" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
Gabriela Dourado: a minha visão participar da construção do padrão do Proxy foi imprescindível, porque além de ter uma grande identificação pela lógica e o papel onde aplicamos o padrão de projeto, é uma das partes mais cruciais para a segurança do ecossistema que estamos desenvolvendo. O Proxy atua como a primeira linha de defesa do nosso backend. Codificar as travas de autenticação, limite diário de relatos e filtro de vocabulário deixou muito claro para mim como um padrão estrutural pode resolver problemas reais de negócio. Sem essa validação intermediária, o banco de dados ficaria vulnerável a sobrecargas e a plataforma perderia a credibilidade das avaliações turísticas.
...
| 19/05/2026 | `1.7`  | Inclusão do link do arquivo `.drawio` (Google Drive) da estrutura do Proxy na versão 2 | [Amanda De Moura](https://github.com/AmandaaMoura) | Gabriela, Mariana e Milena |
| 19/05/2026 | `1.8`  | Preenchimento da seção *Revisões* com o print e o resumo da revisão da Mariana | [Amanda De Moura](https://github.com/AmandaaMoura) | Gabriela, Mariana e Milena |
| 20/05/2026 | `1.9`  | Correção/ordenação das referenciações e fontes das imagens conforme revisão da colega Milena | [Gabriela Dourado](https://github.com/gabrieladouradof) | Gabriela, Mariana e Milena 
| 20/05/2026 | `1.10`  | Preenchimento da seção *Revisões* com o print e o resumo da revisão da Milena | [Gabriela Dourado](https://github.com/gabrieladouradof) | Gabriela, Mariana e Milena |
| 21/05/2026 | `1.11`  | Inclusão da seção *Vídeo de demonstração do Proxy* com iframe do YouTube | [Amanda De Moura](https://github.com/AmandaaMoura) | Gabriela, Mariana e Milena |
| 21/05/2026 | `1.12`  | Inclusão de referências pontuais (ABNT autor-ano) na introdução, conectando conceitos do padrão Proxy às referências [1] Refactoring Guru, [2] Serrano e [3] SourceMaking | [Amanda De Moura](https://github.com/AmandaaMoura) | Gabriela, Mariana e Milena |
