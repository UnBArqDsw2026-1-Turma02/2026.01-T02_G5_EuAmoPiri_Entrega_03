...
5. **Revisão dos Revisores:** Os Revisores Mariana e Samuel discorreram sobre seu ponto de vista em relação a solução que implementamos, pode-se observar nas imagens a seguir
![Figura 4: Revisão Mariana](imagens/revisao_mari.png)

![Figura 5: Revisão Samuel](imagens/revisao_samuel.png)
...
O diagrama abaixo apresenta a implementação do padrão Strategy no sistema

![Figura 5: Validação final do código e execução](imagens/imagem5.png)

O diagrama mostra a relação entre a classe principal ListaLocais, a interface IEstrategiaOrdenacao e as estratégias concretas de ordenação. A classe ListaLocais mantém uma referência para uma estratégia de ordenação e utiliza a interface IEstrategiaOrdenacao para executar o comportamento desejado, sem depender diretamente de uma implementação específica. As classes OrdenacaoPorNome, OrdenacaoPorAvaliacao e OrdenacaoPorDistancia implementam essa interface, cada uma contendo uma lógica própria de ordenação dos locais turísticos. As setas tracejadas representam a implementação da interface pelas estratégias concretas, enquanto a seta contínua indica a associação entre ListaLocais e a estratégia utilizada.
...
![Figura 6: Discussão da equipe via Discord](imagens/image.webp)
...
Milena: Durante a concepção do artefato, buscamos desenvolver uma solução que fosse flexível, organizada e de fácil manutenção. Discutimos maneiras de evitar estruturas condicionais extensas e identificou que o padrão Strategy seria a melhor abordagem para separar as diferentes formas de ordenação dos locais turísticos. Além disso, contribuímos na definição das responsabilidades de cada classe, na implementação das estratégias e na validação do funcionamento da aplicação, permitindo uma melhor compreensão prática dos benefícios arquiteturais proporcionados pelo padrão.

Letícia: na minha visão, o padrão strategy foi um dos mais tranquilos de pensar dentro do projeto, porque o problema das diferentes formas de ordenação já combinava bastante com a proposta do padrão. a videoaula da professora ajudou bastante a entender melhor o conceito e visualizar como ele poderia ser usado na prática.


# Vídeo Apresentação

<iframe width="560" height="315" 
src="https://www.youtube.com/embed/uWNk4GpgpgA" 
