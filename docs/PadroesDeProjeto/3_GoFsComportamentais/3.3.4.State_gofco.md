...
Alternativamente, copie o conteúdo de `state.ts` para um interpretador online que suporte TypeScript, como o [TS Runner do OneCompiler](https://onecompiler.com/typescript), e execute o script.
...
**Leticia(revisora)**: Achei interessante como vocês conseguiram deixar a aplicação do State bem natural dentro do contexto do comentário. Conforme fui lendo, deu para perceber bem a lógica dos estados e como cada comportamento foi sendo separado sem deixar o fluxo confuso. Também gostei da forma como documentaram as decisões e refinamentos ao longo do processo, porque isso ajuda bastante a entender o raciocínio por trás da modelagem e mostra que as escolhas não foram feitas de forma aleatória. Ficou um material bem consistente e fácil de acompanhar!

**João Victor(revisor)**: Dei uma estudada aqui no conteúdo a respeito de State, e revisando o código aqui acho que fizeram um ótimo trabalho na implementação! Eu não esperava que tudo fosse ser aplicado em um arquivo só, então fiquei curioso conforme revisava já que por aqui estou mais acostumado a separar em arquivos individuais. E vendo como desenvolveram e organizaram, gostei de como ficou bem intuitivo de acompanhar. Concordo com a forma que aplicaram a questão dos estados e comportamento do sistema por meio das classes usadas, se encaixou bem com o contexto do projeto, e os comentários ajudaram muito na visualização.
...
> **Nota:** o método `setState()` apresentado no vídeo ( no diagrama UML) foi **renomeado posteriormente à gravação** para `mudarEstado()` no Context (`Comentario`), com o objetivo de eliminar a ambiguidade semântica em relação ao `setState()` dos estados concretos. A versão consolidada do artefato (UML v1.1 e implementação TypeScript v1.1) já reflete essa nomenclatura, conforme registrado nas seções *Estrutura — Versão 1.1*, *Implementação - Versão 1.1* e nas entradas correspondentes do *Histórico do artefato*.
...
| 21/05/2026 | 1.8   | Inclusão de nota após o vídeo informando que `setState()` foi renomeado para `mudarEstado()` posteriormente à gravação | [Amanda de Moura](https://github.com/AmandaaMoura)         | [Eduardo Ribeiro](https://github.com/EduardoRibeiroXavier), [Letícia Paiva](https://github.com/leticiakrpaiva), [João Victor Mello](https://github.com/Chaotzuu) |
| 21/05/2026 | 1.9   | Inclusão da visão do revisor João Victor na seção *Visão dos contribuidores na concepção do artefato* | [Amanda de Moura](https://github.com/AmandaaMoura)         | [Eduardo Ribeiro](https://github.com/EduardoRibeiroXavier), [Letícia Paiva](https://github.com/leticiakrpaiva), [João Victor Mello](https://github.com/Chaotzuu) |
| 21/05/2026 | 2.0   | Inclusão da visão da revisora Letícia Paiva na seção *Visão dos contribuidores na concepção do artefato* | [Letícia Paiva](https://github.com/leticiakrpaiva)         | [Amanda de Moura](https://github.com/AmandaaMoura), [Eduardo Ribeiro](https://github.com/EduardoRibeiroXavier), [João Victor Mello](https://github.com/Chaotzuu) |
